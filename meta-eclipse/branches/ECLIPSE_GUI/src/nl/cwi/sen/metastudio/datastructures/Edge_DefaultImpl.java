...
extends Edge
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeEdge_Default(fun, i_args, annos);
  }
  ...
  static public Edge fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Edge tmp = getStaticDatastructuresFactory().makeEdge_Default(NodeId.fromTerm( (aterm.ATerm) children.get(0)), NodeId.fromTerm( (aterm.ATerm) children.get(1)), AttributeList.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
