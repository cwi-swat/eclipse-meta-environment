...
extends Node
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeNode_Default(fun, i_args, annos);
  }
  ...
  static public Node fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Node tmp = getStaticDatastructuresFactory().makeNode_Default(NodeId.fromTerm( (aterm.ATerm) children.get(0)), AttributeList.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
