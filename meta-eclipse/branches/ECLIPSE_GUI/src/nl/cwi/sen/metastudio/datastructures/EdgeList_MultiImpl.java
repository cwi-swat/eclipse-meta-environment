...
extends EdgeList
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeEdgeList_Multi(fun, i_args, annos);
  }
  ...
  static public EdgeList fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      EdgeList tmp = getStaticDatastructuresFactory().makeEdgeList_Multi(Edge.fromTerm( (aterm.ATerm) children.get(0)), EdgeList.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
