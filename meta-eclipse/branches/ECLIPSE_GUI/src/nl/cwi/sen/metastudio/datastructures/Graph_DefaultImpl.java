...
extends Graph
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeGraph_Default(fun, i_args, annos);
  }
  ...
  static public Graph fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Graph tmp = getStaticDatastructuresFactory().makeGraph_Default(NodeList.fromTerm( (aterm.ATerm) children.get(0)), EdgeList.fromTerm( (aterm.ATerm) children.get(1)), AttributeList.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
