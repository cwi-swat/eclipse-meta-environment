...
extends NodeList
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeNodeList_Multi(fun, i_args, annos);
  }
  ...
  static public NodeList fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      NodeList tmp = getStaticDatastructuresFactory().makeNodeList_Multi(Node.fromTerm( (aterm.ATerm) children.get(0)), NodeList.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
