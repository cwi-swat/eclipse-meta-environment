...
extends Attribute
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeAttribute_BoundingBox(fun, i_args, annos);
  }
  ...
  static public Attribute fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Attribute tmp = getStaticDatastructuresFactory().makeAttribute_BoundingBox(Point.fromTerm( (aterm.ATerm) children.get(0)), Point.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
