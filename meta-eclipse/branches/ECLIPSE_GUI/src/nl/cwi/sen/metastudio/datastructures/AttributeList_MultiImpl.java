...
extends AttributeList
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makeAttributeList_Multi(fun, i_args, annos);
  }
  ...
  static public AttributeList fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      AttributeList tmp = getStaticDatastructuresFactory().makeAttributeList_Multi(Attribute.fromTerm( (aterm.ATerm) children.get(0)), AttributeList.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
