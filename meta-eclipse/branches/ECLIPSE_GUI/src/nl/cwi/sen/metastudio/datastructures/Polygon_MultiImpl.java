...
extends Polygon
{
  ...
  protected aterm.ATermAppl make(aterm.AFun fun, aterm.ATerm[] i_args, aterm.ATermList annos) {
    return getDatastructuresFactory().makePolygon_Multi(fun, i_args, annos);
  }
  ...
  static public Polygon fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Polygon tmp = getStaticDatastructuresFactory().makePolygon_Multi(Point.fromTerm( (aterm.ATerm) children.get(0)), Polygon.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
