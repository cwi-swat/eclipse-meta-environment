...
#    warning "Detected an integer size other then 4 or 8 bytes, this can lead to undefined behaviour."
#  endif
#endif
 ...
 */
static unsigned int signedToUnsignedInt(int signedInt){
	union _Integer{
		int s;
		unsigned int u;
	} Integer;
 ...
 */
static int unsignedToSignedInt(unsigned int unsignedInt){
	union _Integer{
		int s;
		unsigned int u;
	} Integer;
